...
routes.get('/admin-login',  (req, res) => {
  let error = req.flash('fail');
  let success_logout = req.flash('success_logout');
  res.render('admin/admin-login/admin_login.ejs', { error, success_logout });
});
...
routes.post("/submit-details",async (req,res)=>{
  ...
  try{
    let result = await fs.readFileSync("admin.json");
    ...
    if(result.email==req.body.email && result.password == req.body.password){
      ...
      res.render('admin/admin-login/admin_login.ejs',{error:"UserName or Password is Incorrect"})
    }
  }catch(err){
    console.log(err);
    res.render('admin/admin-login/admin_login.ejs',{error:"Error! Contact Server Administrator"})
  }
})
...
routes.get('/change-password-front-admin',middleware_check_login,  (req, res) => {
  console.log("jansdkjn")
  
    res.render('admin/front-admin/change-password.ejs',{error:null,success:null});
});
...
routes.post("/change-password-front-admin",async (req,res)=>{
  console.log(req.body)
  try{
    let result = await fs.readFileSync("admin.json");
    result = await JSON.parse(result.toString());
    ...
    if(req.session.user.password == req.body.old_password){
      ...
      if(req.body.new_password==req.body.cnf_password){

        result.password = req.body.new_password;

        await fs.writeFile("admin.json",JSON.stringify(result),(err,data)=>{
              if(err)
                throw err;
            res.render('admin/front-admin/change-password.ejs',{error:null,success:"Password changed and You are logged out from all Previous Devices"})
        });

      }else{
        res.render('admin/front-admin/change-password.ejs',{error:"New password and Confirm Password is Not Same",success:null})
      } 
    }else{
      res.render('admin/front-admin/change-password.ejs',{error:"Old Password is Incorrect",success:null})
    }
  }catch(err){
    console.log(err);
    res.render('admin/front-admin/change-password.ejs',{error:"Error! Contact Server Administrator",success:null})
  }
})
...
//     res.render('admin/front-admin/add-terms-n-conditons', { err_msg, success_msg, Name: req.session.user_name, profile_image: req.session.profile_image, session: req.session, expressFlash: req.flash(), termsAndConditionDetails })
...
routes.get('/add-terms-n-conditons', middleware_check_login, (req, res) => {
  res.render('admin/front-admin/add-terms-n-conditons', {error:null,success:null})
})
...
routes.post('/add-terms-n-conditons', middleware_check_login, (req, res) => {
  ...
  // form.parse(req, (err, fields, files) => {
    ...
    fs.writeFile("terms_condition.json",JSON.stringify(tc),(err,data)=>{
      if(err){
        res.render('admin/front-admin/add-terms-n-conditons', {error:"Could Not update Terms and Conditions!",success:null})
  ...
  routes.get('/get-terms-n-conditions',(req,res)=>{
    fs.readFile("terms_condition.json",(err,data)=>{
        if(err){
          res.send({error:err,sucess:null})
        }else{  
          res.send({error:err,sucess:data.toString()})
        }
    })
  })
...
routes.post('/edit-terms-n-conditons', middleware_check_login, (req, res) => {
  ...
  form.parse(req, (err, fields, files) => {
    ...
    termsAndConditionInfo.updateOne({ _id: id }, {
      ...
      if (err) {
        req.flash('err_msg', 'Something went wrong.');
        res.redirect('/add-terms-n-conditons');
      }
      else {
...
routes.get("/allCompany",(req,res)=>{
  res.render("admin/front-admin/companytable.ejs");
})
